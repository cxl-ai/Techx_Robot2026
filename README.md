...
系统融合了激光雷达稠密环境感知与触控式现代化交互系统，以下为实机运行真实数据与界面效果：

### 4.1 R2 HMI 战术指挥系统交互图谱
基于 PySide6 + QML 构建的高清触控屏车载交互系统，操作手赛前与赛中全流程管控：

#### (1) 红/蓝阵营选择界面 (`BootPage.qml`)
开机引导操作手确认己方队伍，系统毫秒级重构整车坐标系与镜像算法矩阵：
![阵营选择界面](docs/hmi_boot_team.png)

#### (2) 梅林区 12 宫格 A* 寻路地貌标定界面 (`CalibrationPage.qml`)
支持触屏一键绘制假 KFS 障碍、己方真 KFS 与友方 KFS，**内置 A* 启发式算法实时解算最短避障路径（金色边框发光高亮）**：
![梅林区标定与A*规划](docs/hmi_calibration.png)

#### (3) 赛中多阶段战术指挥看板 (`DashboardPage.qml`)
集成武馆区装配卡片、梅林区追踪卡片、对抗区 3x3 九宫格目标多选下发、实时通信延迟监控与**全局硬件急停保护**：
...
#### (1) 比赛场地三维全景高精度激光点云图
利用 `FAST_LIO` 与 Livox MID-360 激光雷达对 2026 赛季标准赛场完成厘米级稠密建图：
![比赛场地全景三维点云图](docs/field_3d_pointcloud.jpg)

#### (2) 三维点云切片转二维栅格地图 (pcd2pgm)
利用 `pcd2pgm` 工具将三维点云按行驶净空高度进行平面切片，生成 Nav2 标准占用栅格地图：
