...
namespace pcd_tools
{
    ...
    {
        open3d::pipelines::registration::RegistrationResult registration_result;
        if (source == nullptr || target == nullptr || source->IsEmpty() || target->IsEmpty())
        {
            open3d::utility::LogWarning(
                "Skip ICP because source/target is empty. source points: {}, target points: {}",
                source == nullptr ? 0 : source->points_.size(),
                target == nullptr ? 0 : target->points_.size());
            return registration_result;
        }
        ...
        else if (icp_method == 1)
        {
            if (!target->HasNormals())
            {
                target->EstimateNormals(
                    open3d::geometry::KDTreeSearchParamHybrid(std::max<double>(icp_distance_threshold, 1e-3) * 2.0, 30));
            }
            registration_result = open3d::pipelines::registration::RegistrationICP(
                *source_transformed, *target, icp_distance_threshold, Eigen::Matrix4d::Identity(),
                                              ...
                                              double voxel_size, int icp_method, std::vector<double> scale)
    {
        ...
        auto pcd_preprocess = [&](int pcd_i)
        {
            ...
            if (vec_pcd_pair[pcd_i].pcd_tgt != nullptr && !vec_pcd_pair[pcd_i].pcd_tgt->IsEmpty())
            {
        ...
        for (int icp_i = 0; icp_i < num_pair; ++icp_i)
        {
            ...
            if (src == nullptr || tgt == nullptr || src->IsEmpty() || tgt->IsEmpty())
            {
                open3d::utility::LogWarning(
                    "Skip multi-scale ICP level {} because source/target is empty. source points: {}, target points: {}",
                    num_pair - 1 - icp_i,
                    src == nullptr ? 0 : src->points_.size(),
                    tgt == nullptr ? 0 : tgt->points_.size());
                continue;
            }
