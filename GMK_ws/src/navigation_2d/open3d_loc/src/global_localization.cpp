...
private:
    ...
    /// @brief lidar_base到baselink
    Eigen::Matrix4d mat_lidarbase2baselink_;
    ...
    double map_publish_frequency_ = 1.0;
                                             ...
                                             tf_listener_(std::make_shared<tf2_ros::TransformListener>(tf_buffer_))
{
    ...
    this->declare_parameter<double>("map_publish_frequency", 1.0);
    ...
    this->get_parameter("map_publish_frequency", map_publish_frequency_);
    ...
    RCLCPP_INFO(this->get_logger(), "  input_odom_topic: %s", input_odom_topic_.c_str());
    RCLCPP_INFO(this->get_logger(), "  input_cloud_topic: %s", input_cloud_topic_.c_str());
    RCLCPP_INFO(this->get_logger(), "  map_publish_frequency: %.3f", map_publish_frequency_);
    ...
    // 注册回调函数，默认对齐 FAST_LIO 的输出话题
    sub_baselink2odom_ = this->create_subscription<nav_msgs::msg::Odometry>(
        input_odom_topic_, 50, std::bind(&GloabalLocalization::CallbackBaselink2Odom, this, std::placeholders::_1));
    sub_scan_cur_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
        input_cloud_topic_, 50, std::bind(&GloabalLocalization::CallbackScan, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(), "Published /map with %zu points", pcd_map_coarse_->points_.size());

    if (map_publish_frequency_ > 0.0)
    {
        auto map_period_ms = std::chrono::milliseconds(
            static_cast<int64_t>(1000.0 / map_publish_frequency_));
    ...
    GetTfTransformToMatrix("base_link", "lidar_base", mat_lidarbase2baselink_);
    std::cout << "mat_lidarbase2baselink_:\n"
              << mat_lidarbase2baselink_ << std::endl;
...
void GloabalLocalization::CallbackBaselink2Odom(const nav_msgs::msg::Odometry::SharedPtr baselink2odom)
{
    ...
    auto mat_lidarbase2odom = mat_current.matrix();

    mat_baselink2odom_ = mat_lidarbase2odom * mat_lidarbase2baselink_.inverse();
            ...
            *target = *map_fine_crop;
            ...
            if (target->IsEmpty() || source->IsEmpty())
            {
                open3d::utility::LogWarning(
                    "Skip localization initialize because cropped cloud is empty. source points: {}, target points: {}, "
                    "OBB_scan center: [{}, {}, {}], OBB_map center: [{}, {}, {}]",
...
{
    RCLCPP_INFO(this->get_logger(), "wait for %s", input_odom_topic_.c_str());
    // 等待接收到第一条里程计消息（通过检查timestamp是否有效）
    while (rclcpp::ok() && timestamp_odom_.seconds() == 0.0)
    {
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Waiting for %s...", input_odom_topic_.c_str());
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    RCLCPP_INFO(this->get_logger(), "Received %s", input_odom_topic_.c_str());

    RCLCPP_INFO(this->get_logger(), "wait for %s", input_cloud_topic_.c_str());
    ...
    while (rclcpp::ok())
    {
        ...
        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "Waiting for %s...", input_cloud_topic_.c_str());
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    RCLCPP_INFO(this->get_logger(), "Received %s", input_cloud_topic_.c_str());
    ...
    while (rclcpp::ok())
    {
        ...
        else
        {
            ...
            *target = *map_fine_crop;
            ...
            {
                open3d::utility::LogWarning(
                    "Skip localization update because cropped cloud is empty. source points: {}, target points: {}, "
                    "OBB_scan center: [{}, {}, {}], OBB_map center: [{}, {}, {}]",
                    source->points_.size(), target->points_.size(),
                    OBB_scan->center_.x(), OBB_scan->center_.y(), OBB_scan->center_.z(),
                    OBB_map->center_.x(), OBB_map->center_.y(), OBB_map->center_.z());
