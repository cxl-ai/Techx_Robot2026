...
def generate_launch_description():
    ...
    relocalization_pkg_path = get_package_share_directory('relocalization')  # 重定位包
    ...
    enable_relocalization = LaunchConfiguration('enable_relocalization')  # 重定位开关
    ...
    declare_config_file_cmd = DeclareLaunchArgument('config_file', default_value='mapping.yaml')   # 配置文件
    ...
    declare_enable_relocalization_cmd = DeclareLaunchArgument('enable_relocalization', default_value='true')  # 是否储存子图
    declare_ros2bag_path_cmd = DeclareLaunchArgument('ros2bag_path', default_value='/home/lhl/rosbags/rosbag2_RUIXING')             # ros2bag包
    ...
    relocalization_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            os.path.join(relocalization_pkg_path, 'launch', 'mapping.launch.py')
        ]),
    ...
    ld.add_action(declare_enable_relocalization_cmd)
    ...
    ld.add_action(relocalization_launch)
