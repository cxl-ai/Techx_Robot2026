...
#include <deque>
#include "relocalization/msg/pose6_d.hpp"
...
typedef relocalization::msg::Pose6D Pose6D;
...
{
    // 直接转换为int64_t获取秒部分，避免std::floor的精度损失
    int64_t sec = static_cast<int64_t>(timestamp);
    // 计算纳秒部分，确保精度
    uint32_t nanosec = static_cast<uint32_t>((timestamp - static_cast<double>(sec)) * 1e9);
