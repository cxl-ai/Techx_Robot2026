...
class ImuProcess
{
 public:
  ...
  Eigen::Matrix3d m_init_R;
  Eigen::Vector3d m_init_t;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, int &N)
{
  ...
  std::cout<<(mean_acc / mean_acc.norm()).transpose()<<std::endl;

  //state_inout.rot = Eye3d; // Exp(mean_acc.cross(V3D(0, 0, -1 / scale_gravity)));
  init_state.rot = SO3(m_init_R);
  init_state.pos = m_init_t;
...
void ImuProcess::Process(const MeasureGroup &meas,  esekfom::esekf<state_ikfom, 12, input_ikfom> &kf_state, PointCloudXYZI::Ptr cur_pcl_un_)
{
  ...
  if (imu_need_init_)
  {
    ...
    if (init_iter_num > MAX_INI_COUNT)
    {
      ...
      // RCLCPP_INFO("IMU Initial Done: Gravity: %.4f %.4f %.4f %.4f; state.bias_g: %.4f %.4f %.4f; acc covarience: %.8f %.8f %.8f; gry covarience: %.8f %.8f %.8f",\
      //          imu_state.grav[0], imu_state.grav[1], imu_state.grav[2], mean_acc.norm(), cov_bias_gyr[0], cov_bias_gyr[1], cov_bias_gyr[2], cov_acc[0], cov_acc[1], cov_acc[2], cov_gyr[0], cov_gyr[1], cov_gyr[2]);
      fout_imu.open((root_dir + "/Log/imu.txt"),ios::out);
    }
